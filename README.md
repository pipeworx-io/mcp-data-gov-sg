...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/data_gov_sg_search_datasets \
  -H 'Content-Type: application/json' \
  -d '{"query":"rainfall"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/data_gov_sg_search_datasets`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "data-gov-sg": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-data-gov-sg"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-data-gov-sg
```
